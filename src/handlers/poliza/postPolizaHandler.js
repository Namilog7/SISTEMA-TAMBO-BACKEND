const { PolizaSeguro, conn } = require("../../db");
...
const postPolizaHandler = async (req, res) => {
    ...
    try {
        ...
        const poliza = await postPoliza.create(
            { numero_poliza, nombre, seccion, desde, hasta, importe, estado, cantidad_cuotas, foto_factura, afectados },
            { transaction }
        );
